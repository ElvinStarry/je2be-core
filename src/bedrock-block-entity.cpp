...
public:
  ...
  static std::optional<Result> FromBlockAndBlockEntity(
      ...
      bool item) {
    using namespace std;
    if constexpr (kJavaDataVersion >= (int)JavaDataVersions::Release26_2) {
      // Legacy Bedrock chunks keep the Java 1.17 chunk layout, but their end
      // gateways still need the destination schema used by the 26.2 world.
      if (outputDataVersion < kJavaDataVersionComponentIntroduced && Namespace::Remove(block.fName) == u8"end_gateway") {
        Options o;
        o.fOutputDataVersion = kJavaDataVersion;
        o.fItem = item;
        return EndGateway(pos, block, tag, blockJ, ctx, o);
      }
    }
      ...
      for (auto const &it : *occupants) {
        ...
        if (result && result->fEntity) {
          ...
          for (auto const &key : {u8"Air", u8"ArmorDropChances", u8"ArmorItems", u8"Brain", u8"UUID",
                                  ...
                                  u8"FallDistance", u8"fall_distance", u8"HandItems", u8"FallFlying", u8"HurtTime", u8"NoGravity",
